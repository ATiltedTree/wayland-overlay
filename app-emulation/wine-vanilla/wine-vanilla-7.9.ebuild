...
SRC_URI="${SRC_URI}
	...
	https://github.com/ATiltedTree/wine-wayland/archive/refs/tags/wine-${PV}.tar.gz -> wine-wayland-${PV}.tar.gz
"

LICENSE="LGPL-2.1"
SLOT="${PV}"
IUSE="+abi_x86_32 +abi_x86_64 +alsa capi cups custom-cflags crossdev-mingw dos +fontconfig +gecko gphoto2 gssapi gstreamer kerberos ldap mingw +mono mp3 netapi nls odbc openal opencl +opengl osmesa oss +perl pcap pulseaudio +realtime +run-exes samba scanner sdl selinux +ssl test +threads +truetype udev +udisks +unwind usb v4l vulkan +X +xcomposite xinerama wayland"
...
src_prepare() {
	...
	if use wayland; then
		eapply "${WORKDIR}"/wine-wayland-wine-${PV}
	fi
